...
played_card_status <- reactive({
  ...
  played_total <- move_fact$data[CARD_ID > 0, .N]

  #check how many moves I need to make this turn in case I am finished
  cyclers_left_total <- game_status_simple()[CYCLER_ID > 0, CYCLER_ID]
  ...
  current_turn <- get_current_turn(input$join_tournament, game_status_simple(), con)
  # #count cards waiting to be played
   count_cards_waiting <- move_fact$data[TEAM_ID == player_reactive$team & CARD_ID < 0, .N]
   how_many_played <-  move_fact$data[TEAM_ID == player_reactive$team & CARD_ID > 0  & TURN_ID == current_turn, .N]
   first_cycler_selected <- move_fact$data[TEAM_ID == player_reactive$team & TURN_ID == current_turn , .N] > 0
